...
must_haves:
  ...
  key_links:
    ...
    - from: "src/negotiation/app.py"
      to: "src/negotiation/observability/metrics.py"
      via: "DEALS_CLOSED.inc() on accept, ACTIVE_NEGOTIATIONS.set() on state changes"
      pattern: "DEALS_CLOSED\\.inc|ACTIVE_NEGOTIATIONS\\.set"
---
...
<task type="auto">
  ...
  <action>
    ...
    4. **Instrument business metrics in `src/negotiation/app.py`** (CRITICAL — without this, OBS-03 custom metrics always report 0):
       - Add import at top: `from negotiation.observability.metrics import ACTIVE_NEGOTIATIONS, DEALS_CLOSED`
       - **Startup recovery (after the `for row in active_rows` loop that populates `negotiation_states`):**
         After the recovery loop completes, add:
         ```python
         ACTIVE_NEGOTIATIONS.set(len(negotiation_states))
         ```
       - **New negotiation added (in `initiate_outreach`, after `negotiation_states[thread_id] = {...}`):**
         After storing the new negotiation state, add:
         ```python
         ACTIVE_NEGOTIATIONS.set(len(negotiation_states))
         ```
       - **Deal accepted (in `handle_inbound_email`, in the `elif result["action"] == "accept":` branch):**
         Add:
         ```python
         DEALS_CLOSED.inc()
         ACTIVE_NEGOTIATIONS.set(len(negotiation_states))
         ```
         Note: The state_store excludes terminal states (AGREED/REJECTED) from `load_active()`, so `negotiation_states` dict count reflects active negotiations. If the code removes terminal threads from `negotiation_states` after acceptance, the gauge auto-updates; if not, the gauge still reflects the in-memory count accurately.
       - **Deal rejected (in `handle_inbound_email`, in the `elif result["action"] == "reject":` branch, if applicable):**
         Add:
         ```python
         ACTIVE_NEGOTIATIONS.set(len(negotiation_states))
         ```
    ...
    5. Create `tests/test_metrics.py`:
       ...
       - Test `test_metrics_endpoint_returns_prometheus_format`: `GET /metrics` returns 200 with content containing `http_request` and `negotiation_active_total` and `negotiation_deals_closed_total`
       - Test `test_excluded_handlers_not_in_metrics`: `/health` and `/ready` do NOT appear in metrics output (excluded_handlers works)
       - Test `test_active_negotiations_gauge_changes`: Import `ACTIVE_NEGOTIATIONS`, call `.set(5)`, read `/metrics`, assert `negotiation_active_total 5.0` appears in response text; then `.set(3)`, read again, assert `negotiation_active_total 3.0`
       - Test `test_deals_closed_counter_increments`: Import `DEALS_CLOSED`, record initial value from `/metrics`, call `.inc()`, read `/metrics` again, assert `negotiation_deals_closed_total` increased by 1.0

    6. Create `tests/test_request_id.py`:
